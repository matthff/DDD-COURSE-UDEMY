...
namespace DDD_Api_Test.PostalCodeControllerTest.POST
{
    public class TestCreatedResult
    {
        ...
        public async Task MustReturnCreateResult()
        {
            ...
            var resultValue = ((CreatedResult)result).Value as PostalCodeCreateResultDTO;
            Assert.NotNull(resultValue);
            Assert.Equal(postalCodeCreateDTO.PostalCode, resultValue.PostalCode);
            Assert.Equal(postalCodeCreateDTO.Address, resultValue.Address);
            Assert.Equal(postalCodeCreateDTO.StreetNumber, resultValue.StreetNumber);
            Assert.Equal(postalCodeCreateDTO.CityId, resultValue.CityId);
        }
    }
}